...
def featureAdd2(samples, flows, sr_ratio):
    """
    samples: Tensor(b, h, w) float32
    flows: Tensor(b, 2, h, w) float32
    """
    ...
    mapping = sr_ratio*(flows_ + get_coords(h,w).cuda())
    mappingy, mappingx = torch.split(mapping, 1, dim = 1)
    mappingy = torch.clamp(mappingy, 0, sr_ratio*h-1)
    mappingx = torch.clamp(mappingx, 0, sr_ratio*w-1)
    ...
    diff = (mapping - coords_ul).type(torch.float32).cuda()
    neg_diff = (1.0 - diff).type(torch.float32).cuda()
    diff_y, diff_x = torch.split(diff, 1, dim = 1)
    neg_diff_y, neg_diff_x = torch.split(neg_diff, 1, dim = 1)
    diff_x = diff_x.view(b,-1)
    diff_y = diff_y.view(b,-1)
    ...
    coords_ul = coords_unroll(coords_ul, sr_ratio).type(torch.long).cuda()
    coords_ur = coords_unroll(coords_ur, sr_ratio).type(torch.long).cuda()
    coords_ll = coords_unroll(coords_ll, sr_ratio).type(torch.long).cuda()
    coords_lr = coords_unroll(coords_lr, sr_ratio).type(torch.long).cuda()

    dadd = torch.zeros(b, sr_ratio*sr_ratio*h*w).cuda()
