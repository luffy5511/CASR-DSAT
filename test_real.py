...
from MEnet import TransformerME, FNet
...
from real_dataset.burstsr_dataset import BurstSRDataset, BurstSRDataset_real
...
class DeepSR(nn.Module):
    ...
    def forward(self, samplesLR, flag_train, c, losstype):
        ...
        flow, warploss, warped = flowEstimation(samplesLR, self.MEAlign, c,
                                                ...
                                                gaussian_filter=GaussianBlur(11, sigma=1), losstype=losstype)  # 待室外
        b, num_im, h, w = samplesLR.shape
        ...
        SR = self.APGU(self.PBFF(dadd)).squeeze(
...
"""Test real camera array datasets using fine-tuned CASR-DAST model"""
def test(args):
    ...
    checkpoint_path = "/Real-CASR-DSAT-GM2M.pth.tar"
    ...
    Dataset_path = "/test_real_burst/"  # Make sure to preprocess the downloaded data first
    val_data_set = BurstSRDataset_real(root=Dataset_path, split='test', burst_size=9, crop_sz=(112, 112), center_crop=True,
                                   random_flip=False)  # 实拍
    val_loader = torch.utils.data.DataLoader(val_data_set, batch_size=val_bs,
                                             num_workers=0, shuffle=False)
    result_dir_SR ="/test_real_result/"
    ##################
    starttime = time()
    ##################
    with torch.no_grad():
        for k, data in enumerate(val_loader):
            ...
            imwrite(os.path.join(result_dir_SR, "SR_{:03d}.png".format(k)), SR)
            '''warped = warped.view(b, num_im, h, w)
            for n in range(num_im):
                LR = samplesLR[0, n, ...]
                LR = LR.detach().cpu().numpy().squeeze()
                imwrite(os.path.join(result_dir_SR, "LR_{:03d}_{:02d}.png".format(k, n)), LR)
                warp = warped[0, n, ...]
                warp = warp.detach().cpu().numpy().squeeze()
                imwrite(os.path.join(result_dir_SR, "Warped_{:03d}_{:02d}.png".format(k, n)), warp)'''
