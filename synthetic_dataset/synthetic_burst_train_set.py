...
from synthetic_dataset.synthetic_burst_generation import hr2burst, random_crop, hr2burst_full  # syn_burst_utils
import cv2
...
class SyntheticBurst(torch.utils.data.Dataset):
    ...
    def __init__(self, base_dataset, burst_size=9, crop_sz=384, gray_max=255, split='train'):
        self.base_dataset = base_dataset

        self.burst_size = burst_size
        self.crop_sz = crop_sz

        self.downsample_factor = burst_size ** (1 / 2)
        self.burst_transformation_params = {'max_translation': 24.0,  # 待确定
                                                  'max_rotation': 1.0,
                                                  'max_shear': 0.0,
                                                  'max_scale': 0.0,
                                                  'border_crop': 24}
    ...
    def __getitem__(self, index):
        ...
        # frame = tfm.Grayscale(num_output_channels=1)(frame)
        ...
        # Generate RAW burst
        burst, frame_gt, flow_vectors = hr2burst(frame_crop,
                                                 self.burst_size,
                                                 self.downsample_factor,
                                                 self.burst_transformation_params,
                                                 interpolation_type=self.interpolation_type, gray_max=self.gray_max
                                                 )
        ...
        '''if self.split == 'train':
            burst_full, frame_gt_full, flow_vectors = hr2burst_full(frame, translations, thetas, shear_factors,
                                                                    ar_factors, scale_factors,
                                                                    self.burst_size,
                                                                    self.downsample_factor,
                                                                    interpolation_type=self.interpolation_type,
                                                                    gray_max=self.gray_max
                                                                    )
            r1 = int((r1 + self.burst_transformation_params.get('border_crop',0))/self.downsample_factor)
            c1 = int((c1 + self.burst_transformation_params.get('border_crop', 0))/self.downsample_factor)
            return burst, frame_gt, burst_full, r1, c1
        else:
